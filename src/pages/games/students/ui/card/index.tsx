...
const SpellCard: FC<gameCardProps> = ({ url, title, img }) => {
  ...
  useEffect(() => {
    const mode = localStorage.getItem("StudentsdModeEnglish");
    setSaveMode(mode);
  }, []);
  ...
  return (
    <>
      <Card className="card shadow-[4.0px_8.0px_8.0px_rgba(0,0,0,0.38)] mb-2 disabled w-[80%] md:w-1/4 l:w-1/5 lg:w-1/5  bg-[#0284c7] glass mx-auto ">
        <Card.Body className="items-center text-center p-3">
          <div className="h-2/3">
            <Card.Image
          ...
          <h1 className="font-arena lowercase text-4xl leading-6 text-neutral-50 pt-3">
            {title}
          </h1>
